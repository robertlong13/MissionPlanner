...
namespace MissionPlanner.ArduPilot.Mavlink
{
    ...
    {
        public Dictionary<byte, MAVLink.mavlink_gimbal_manager_information_t> ManagerInfo =
        ...
        public Dictionary<byte, MAVLink.mavlink_gimbal_manager_status_t> ManagerStatus =
            new Dictionary<byte, MAVLink.mavlink_gimbal_manager_status_t>();

        public Dictionary<byte, MAVLink.mavlink_gimbal_device_attitude_status_t> GimbalStatus =
            new Dictionary<byte, MAVLink.mavlink_gimbal_device_attitude_status_t>();
        ...
        public void Discover()
        {
            ...
            mavint.OnPacketReceived += (sender, message) =>
            {
                ...
                if (message.msgid == (uint)MAVLink.MAVLINK_MSG_ID.GIMBAL_DEVICE_ATTITUDE_STATUS)
                {
                    ...
                    lock (GimbalStatus)
                    {
                        GimbalStatus[gds.target_system] = gds;
                    }
                }
            };
        }
        ...
        {
            if (!ManagerInfo.ContainsKey(gimbal_device_id))
            {
                return false;
            }
            ...
            return (ManagerInfo[gimbal_device_id].cap_flags & (uint)flags) != 0;
        }
        ...
        public bool HasAllCapability(MAVLink.GIMBAL_MANAGER_CAP_FLAGS flags, byte gimbal_device_id = 0)
        {
            ...
            return (ManagerInfo[gimbal_device_id].cap_flags & (uint)flags) == (uint)flags;
        }
        ...
        public bool SetROILocation(double lat, double lon, float alt = 0, byte gimbal_device_id = 0, MAVLink.MAV_FRAME frame = MAVLink.MAV_FRAME.GLOBAL_TERRAIN_ALT)
        {
